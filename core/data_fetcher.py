...
def _parse_cn_list_datetime(raw_value: Any, now: datetime.datetime) -> Optional[datetime.datetime]:
...
def _fetch_sec_edgar_filings(
    ...
    if not settings.SEC_WATCHLIST_TICKERS:
        record_data_source_health(source_name, "skipped", "未配置 SEC_WATCHLIST_TICKERS", 0)
        return []
...
def get_news(
    ...
    *,
    ...
    try:
        ...
        second_batch_news = _fetch_second_batch_news(minutes_lookback)

        merged_news = valid_news + normalized_external_news + second_batch_news
        ...
        second_batch_news = _fetch_second_batch_news(minutes_lookback)
        merged_news = normalized_external_news + second_batch_news
        merged_news.sort(key=lambda x: x["datetime"], reverse=True)
        refined_news = _refine_news(merged_news)
        if semantic_dedup:
            refined_news = _deduplicate_semantic_news(refined_news)
        ...
        log_info(
            ...
            f"rss_count={len(normalized_external_news)}, "
            f"second_batch_count={len(second_batch_news)}, "
            f"merged_count={len(merged_news)}, "
