...
_NEWS_TRACK_STOP_WORDS = {
    "news", "market", "markets", "report", "reports", "said", "says", "will",
    "with", "from", "that", "this", "市场", "消息", "新闻", "全球", "重要",
    "最新", "报道", "表示", "发布", "公司", "中国", "美国", "相关", "事件",
    "风险", "影响",
}
...
def _format_news_tracking_update(
    ...
    lines = [
        "🧭 事件跟踪更新",
        f"原事件：{_compact_alert_text(tracker.get('title'), 160)}",
        "",
        "【新进展】",
    ]
    for item in related_items[:2]:
        lines.append(f"- {_compact_alert_text(item.get('title'), 180)}")
        lines.append(f"  来源：{_compact_alert_text(item.get('source'), 60)}")
    ...
    lines.extend(
        (
            "",
            "说明：这是按标题关键词匹配到的新来源，需以原文核实，未确认即不等于事件升级。",
        )
    )
    return "\n".join(lines)
...
def _format_news_tracking_end(tracker: dict[str, Any]) -> str:
    ...
    return "\n".join(
        (
            "⚪️ 事件跟踪结束",
            f"原事件：{_compact_alert_text(tracker.get('title'), 160)}",
            f"结果：已完成 2 小时跟踪，期间推送 {updates} 条相关新进展。",
            "说明：仅覆盖系统已接入的新闻源；未发现更新不代表事件没有后续。",
        )
    )
...
def _process_active_news_trackers(
    ...
    for tracker in store.active_news_trackers(now):
        ...
        related_items = [
            ...
            if not item.get("discovery_only") and _is_related_tracked_news(tracker, item)
        ]
        if related_items:
...
def handle_news_tracking_callback(
    ...
    if not is_authorized_interaction(callback):
    ...
    if action != str(NEWS_TRACK_MINUTES):
        return "不允许的事件跟踪时长。"
    return (
        "已开启 2 小时事件跟踪；系统仍每分钟核对已接入新闻源，有新的相关标题才会推送。"
