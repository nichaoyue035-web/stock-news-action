...
4. **DeepSeek 生成摘要 / 翻译 / 分析**：根据 `prompts.json` 或默认提示词生成结构化内容。
...
| `monitor` | 盘中机会雷达 | 筛选近期开盘/盘中快讯中具有即时交易价值或市场影响的消息，推送到监控频道 |
...
| 变量名 | 作用 | 使用场景 |
| --- | --- | --- |
| `DEEPSEEK_API_KEY` | DeepSeek API Key，用于新闻翻译、摘要和分析 | `daily`、`funds`、`monitor`、`periodic`、`after_market`、`global`、`recommend`、`track` |
